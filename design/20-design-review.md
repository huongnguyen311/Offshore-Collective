...
**[SUGGESTION]** `13-state-gallery.md` — the Notification Center (M06) empty state is a documented exception to the "empty states always need a CTA" rule, and the reasoning given (nothing plausible to point to) is sound. Consider getting a one-line confirmation from Matt that "there's genuinely nothing to do yet" reads correctly to a first-time partner, since this is a judgment call rather than something derivable from the SOW.

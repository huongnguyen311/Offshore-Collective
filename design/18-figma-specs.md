...
| standby | `color.calendar.standby-bg`, 1.5pt stroke `color.calendar.standby-border`, **no StateGlyph**, plus a `TODAY` label above the DayNumber | Tappable → navigates to M28. Mon to Thu only (2026-09-19); never applied to a Fri, Sat or Sun cell. The `TODAY` label is a separate overlay, neutral by default, green only on this state |

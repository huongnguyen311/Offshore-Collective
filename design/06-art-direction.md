...
| Signature component treatment | Booking Calendar (M04) date states | Available/booked/standby/unclaimed/blocked dates, plus a C.7 named-holiday long weekend, distinguished by the brand's rounded-square motif (not generic colored dots) — a small rounded-square glyph per date state, echoing the icon's interlocking squares. There is no `held` state: it was a leftover from the removed Rayglass 2400 towing-approval flow | Ties the calendar — the app's most-used screen — directly to the brand mark instead of a generic calendar library look |

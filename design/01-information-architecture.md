...
| M16 | Post-Use Checklist | Action | (triggered post-trip, from M06/M09) | M16 submitted state, then M03 | Yes | Submitting closes the trip and starts the turnaround, and lands on a confirmation state rather than dropping straight to Home (2026-09-19). That state carries the A.11 fuel acknowledgement when the tank was reported as not full |
...
| M16 | **Resolved 2026-09-19** | Post-Use Checklist exited straight to M03 with no explicit confirmation screen, so the two things the submission does, closing the trip and starting the turnaround, were never stated to the partner | Drawn as its own state. It also gave the A.11 fuel acknowledgement somewhere to live, which is what forced the issue |
...
| M06 | **Resolved 2026-09-19** | The Christmas Window release notification (C.8/C.9) was the one row with no destination: M13 showed only your own window and M04 carried no Christmas state, so there was nowhere to claim a released one | Neither option this audit proposed was taken. A released window is not a thing to be claimed, it is seven ordinary available dates, so the row deep-links to M04 and the standard first-confirmed flow books them. No seventh calendar state, no claimable block on M13, nothing new built |
